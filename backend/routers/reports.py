...
def gap_analysis(db: Session = Depends(get_db), _=Depends(require_staff)):
    return report_service.generate_gap_analysis(db)

@router.get("/quarterly-kpi")
def quarterly_kpi(year: Optional[int] = Query(None), db: Session = Depends(get_db), _=Depends(require_staff)):
