...
def create_item(body: ItemCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def update_item(item_id: int, body: ItemUpdate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return item_service.update_item(db, item_id, body)

@router.delete("/{item_id}", status_code=204)
def deactivate_item(item_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
