...
def create_ncr(body: NCRCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    return ncr_service.create_ncr(db, body, current_user.id)

@router.get("/{ncr_id}", response_model=NCROut)
def get_ncr(ncr_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return ncr_service.get_ncr(db, ncr_id)

@router.put("/{ncr_id}", response_model=NCROut)
def update_ncr(ncr_id: int, body: NCRUpdate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return ncr_service.update_ncr(db, ncr_id, body)

@router.patch("/{ncr_id}/status", response_model=NCROut)
def patch_status(ncr_id: int, body: NCRStatusPatch, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return ncr_service.patch_status(db, ncr_id, body.status)

@router.delete("/{ncr_id}", status_code=204)
def delete_ncr(ncr_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
    ncr_service.delete_ncr(db, ncr_id)

@router.post("/{ncr_id}/comments", response_model=NCRCommentOut, status_code=201)
def add_comment(ncr_id: int, body: NCRCommentCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
...
def download_attachment(ncr_id: int, attachment_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def delete_attachment(ncr_id: int, attachment_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
