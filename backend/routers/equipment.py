...
def download_calibration_template(_=Depends(get_current_user)):
...
def create_investment(body: InvestmentCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return equipment_service.create_investment(db, body)

@router.put("/investments/{inv_id}", response_model=InvestmentOut)
def update_investment(inv_id: int, body: InvestmentCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return equipment_service.update_investment(db, inv_id, body)

@router.delete("/investments/{inv_id}", status_code=204)
def delete_investment(inv_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def create_equipment(body: EquipmentCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def update_equipment(eq_id: int, body: EquipmentUpdate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return equipment_service.update_equipment(db, eq_id, body)

@router.delete("/{eq_id}", status_code=204)
def delete_equipment(eq_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
    equipment_service.delete_equipment(db, eq_id)

# ── 교정 이력 ──────────────────────────────────────────
@router.post("/{eq_id}/calibrations", response_model=CalibrationOut, status_code=201)
def add_calibration(eq_id: int, body: CalibrationCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return equipment_service.add_calibration(db, eq_id, body)

@router.put("/{eq_id}/calibrations/{cal_id}", response_model=CalibrationOut)
def update_calibration(eq_id: int, cal_id: int, body: CalibrationCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return equipment_service.update_calibration(db, eq_id, cal_id, body)

@router.delete("/{eq_id}/calibrations/{cal_id}", status_code=204)
def delete_calibration(eq_id: int, cal_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
    equipment_service.delete_calibration(db, eq_id, cal_id)

# ── 규격 Capability 매핑 ─────────────────────────────────
@router.get("/{eq_id}/standard-items")
def get_standard_items(eq_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
