...
def deactivate_customer(customer_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def add_contact(customer_id: int, body: ContactCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return customer_service.add_contact(db, customer_id, body)

@router.delete("/{customer_id}/contacts/{contact_id}", status_code=204)
def remove_contact(customer_id: int, contact_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def delete_attachment(customer_id: int, attachment_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
