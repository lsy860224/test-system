...
def add_milestone(project_id: int, body: MilestoneCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def get_project_standard_items(project_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def set_project_standard_items(project_id: int, body: StandardItemIds, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def get_project_schedule_detail(project_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
