...
from schemas.schedule import TestScheduleCreate, TestScheduleUpdate, TestScheduleOut, TestScheduleResultPatch, PaginatedSchedules, VendorCreate, VendorOut
...
def create_schedule(body: TestScheduleCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
...
def update_schedule(schedule_id: int, body: TestScheduleUpdate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return schedule_service.update_schedule(db, schedule_id, body)

@router.patch("/{schedule_id}/start", response_model=TestScheduleOut)
def start_schedule(schedule_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return schedule_service.start_schedule(db, schedule_id)

@router.patch("/{schedule_id}/result", response_model=TestScheduleOut)
def record_result(schedule_id: int, body: TestScheduleResultPatch, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
...
def list_vendors(db: Session = Depends(get_db), _=Depends(get_current_user)):
    return schedule_service.list_vendors(db)
...
def create_vendor(body: VendorCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return schedule_service.create_vendor(db, body)
