...
def delete_vendor(vendor_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
    vendor_service.delete_vendor(db, vendor_id)

# ── 시험 범위 (단가표) ─────────────────────────────────────
@router.post("/{vendor_id}/scopes", response_model=TestScopeOut, status_code=201)
def add_scope(vendor_id: int, body: TestScopeCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return vendor_service.add_test_scope(db, vendor_id, body)

@router.put("/{vendor_id}/scopes/{scope_id}", response_model=TestScopeOut)
def update_scope(vendor_id: int, scope_id: int, body: TestScopeCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def delete_scope(vendor_id: int, scope_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
...
def add_order(vendor_id: int, body: OrderCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return vendor_service.add_order(db, vendor_id, body)

@router.put("/{vendor_id}/orders/{order_id}", response_model=OrderOut)
def update_order(vendor_id: int, order_id: int, body: OrderCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return vendor_service.update_order(db, vendor_id, order_id, body)

@router.delete("/{vendor_id}/orders/{order_id}", status_code=204)
def delete_order(vendor_id: int, order_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
