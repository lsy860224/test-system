...
**설계 패턴 노트(2026-07-23, graphify 지식그래프 분석으로 발견)**: `.claude/commands/es-add.md`의 "자체/외주 판단 기준"도 이것과 명시적 링크는 없지만 같은 설계 철학을 따른다 — "판단 기준을 사전에 규칙으로 문서화해 즉흥적/임의적 결정을 배제한다"는 패턴. 이 프로젝트에서 판단 기준이 필요한 새 영역(예: 신규 데이터 유형의 저장 위치, 신규 시험의 자체/외주 여부)이 생기면, 매번 새로 고민하지 말고 이 두 사례의 형식(표로 유형을 나열 + 애매하면 사용자 확인)을 재사용한다.
