...
### 환경/설정 변경 후 재검증 원칙 (stale-state 함정) — 2026-07-23, graphify 지식그래프 분석으로 발견

위 `.bat` 인코딩 사고와 `SERVER_GUIDE.md`의 `setx` 레지스트리 사고는 각각 독립적으로 기록됐지만, graphify로 코드베이스를 그래프화한 뒤 두 문서 간 명시적 링크(import·citation)가 전혀 없는데도 의미상 강하게 연결된다는 게 드러났다 — 근본원인이 하나이기 때문이다: **Windows 프로세스는 자신을 띄운 시점의 환경/설정을 그대로 물려받을 뿐, 실행 중에는 재로드하지 않는다.** 그래서 "이미 고쳤다/등록했다"고 믿고 기존에 열려 있던 세션에서 재검증하면, 실제로는 그 세션이 낡은 상태를 보고 있을 뿐인데 거짓 실패(또는 거짓 성공)로 오인하기 쉽다.

| 사고 | 무엇이 stale했나 | 재발방지 |
|---|---|---|
| `.bat` 파일 BOM/CRLF 인코딩 (위 항목, 2026-07-10) | 에디터가 저장한 파일 인코딩이 cmd.exe 파서를 깨뜨림 — 정적 리뷰로는 안 보임 | 새 터미널에서 `cmd /c file.bat` 실제 실행 재검증 |
| `setx` 레지스트리 vs 기존 프로세스 환경변수 (`SERVER_GUIDE.md` "setx의 함정" 절) | `setx`는 레지스트리만 갱신 — 이미 열려 있던 PowerShell/cmd/Claude Code 세션은 옛 환경변수를 그대로 물고 있음 | `setx` 실행 **이후에 새로 연** 터미널에서 재검증 |

**일반 원칙**: `.bat`/환경변수/레지스트리 등 OS 레벨 설정을 변경한 뒤에는, 그 변경 이후에 새로 띄운 프로세스에서 실제로 동작을 재검증하기 전까지 "적용됐다"고 간주하지 않는다. 기존에 열려 있던 터미널·백그라운드 셸에서 나온 실패(또는 성공) 결과는 신뢰하지 않는다 — 앞으로 이 패턴에 해당하는 새 사고가 생기면 이 표에 행을 추가한다.
...
원격 `origin`이 `https://github.com/lsy860224/test-system.git`로 연결돼 있다(2026-07-23 `git push`로 실제 반영 확인 — 아래 "원격 저장소 없음" 기록 이후 사용자가 직접 추가한 것으로 보이며, 이 항목은 그 갱신이다). 로컬 디스크 장애·도난·손상이 나도 이미 푸시된 커밋 이력은 GitHub에 남는다 — 단 푸시 안 된 로컬 커밋·워킹트리 변경은 여전히 유실 대상이므로 원격 존재가 "커밋 없이도 안전하다"는 뜻은 아니다.

**남은 리스크**: 커밋 습관. 과거 2026-07-15 시점에 25개 파일 수정 + 3개 신규 파일이 스테이징 전 상태로 방치된 적이 있었다(스냅샷이라 지금은 다를 수 있음, 2026-07-23 기준 워킹트리는 clean). 원격이 생겼어도 로컬에서 커밋·푸시하지 않은 변경분은 그대로 무방비 상태이므로, 작업 단위가 끝나면 커밋(및 필요 시 푸시)하는 습관이 계속 중요하다.
