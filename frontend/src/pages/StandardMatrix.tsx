...
const ITEM_STATUSES = ['대기', '진행중', '완료', '보류']
...
export default function StandardMatrix() {
  ...
  const selectedItems = useMemo(() => {
    ...
    const filtered = selectedGroup.items.filter((it) => {
      ...
      if (itemFilterStatus && it.status !== itemFilterStatus) return false
      return true
    })
    return sortItems(filtered, itemSort)
  }, [selectedGroup, itemSearch, itemFilterCategory, itemFilterSourceType, itemFilterStatus, itemSort])
  ...
  const itemColumns: Column<StandardItem>[] = [
    { key: 'standard_code', header: '항목 No.', width: 110, sortable: true },
    { key: 'name', header: '시험 항목명', sortable: true },
    {
    ...
    {
      key: 'test_condition_summary', header: '시험 조건', width: 200,
    ...
    {
      key: 'status', header: '상태', width: 90, sortable: true,
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        <h2 style={{ fontSize: 18, fontWeight: 700, flex: 1 }}>
          규격 매트릭스 <span style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-muted)' }}>
            {selectedGroup ? `${selectedGroup.standard_no || '(규격 No. 미지정)'} · 시험 항목 ${selectedItems.length}건` : `규격 ${groups.length}종 · 총 ${total}건`}
          </span>
        </h2>
        {!selectedGroup && (
          <>
      ...
      {formItemId !== undefined && (
        <StandardItemForm
          itemId={formItemId}
          onClose={() => { setFormItemId(undefined); setCopyFromStdNo(undefined) }}
          onSaved={handleSaved}
